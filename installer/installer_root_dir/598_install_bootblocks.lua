		...
		for i, dataset in ipairs(response.datasets) do
			if dataset.boot0cfg == "Y" then
				if dataset.usegrub == "Y" then
					...
					cmds:add("echo \"default=0\" > /boot/grub/menu.1st")
					cmds:add("echo \"timeout=5\" >> /boot/grub/menu.1st")
					cmds:add("echo \"title pfSense\" >> /boot/grub/menu.1st")
					cmds:add("echo \"	root (hd0,0,a)\" >> /boot/grub/menu.1st")
					cmds:add("echo \"	kernel /boot/loader\" >> /boot/grub/menu.1st")
					cmds:add("echo \"	makeactive\" >> /boot/grub/menu.1st")
					cmds:add("echo \"	savedefault\" >> /boot/grub/menu.1st")
					cmds:add("/usr/local/sbin/grub-install --root-directory=/mnt/ /dev/${disk}")
					cmds:execute()
				else
					dd = disk_ref[dataset.disk]
